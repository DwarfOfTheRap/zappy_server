...
void	test_snd_overflow(t_player *p, int cr)
{
	u_int		i = 0;
	char		str[SND_SIZE + 1];
	...
	ck_assert_str_eq(p->snd.buf[p->snd.read], str);
	p->snd.read = (p->snd.read + 1 == NB_SND) ? 0 : p->snd.read + 1;
	ck_assert_str_eq(p->snd.lst.first->content, (cr) ? "ca\n" : "ca");
	clean_msg_queue(p);
}
